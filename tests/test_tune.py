...
from redxclassifiers import tune
from redxclassifiers.tune import (
    JoinKfoldData,
...
def test_objective_random_cv_multi_rmse__r2__diff_train_test(trial):
    ...
    need to use the same number of values for each set hence need an even number of inputs and even number of folds which is exactly divisible
    """
    regressorfx = Mock()
...
def test_objective_predefined_cv_multi_rmse__r2__diff_train_test(trial):
    ...
    assert (
        tune.objective_predefined_cv_multi_rmse__r2__diff_train_test(
            trial, regressorfx, parameters, predefined_kfolds
        )
        == expected_output
    )
